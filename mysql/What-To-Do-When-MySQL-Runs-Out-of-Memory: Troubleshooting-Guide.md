...
崩溃故障诊断绝不是一个有趣的任务,尤其如果MySQL没有报告崩溃的原因时,比如,MySQL运行时内存溢出。 Peter Zaitsev 在2012年写了一篇文章[Troubleshooting MySQL Memory Usage ](https://www.percona.com/blog/2012/03/21/troubleshooting-mysql-memory-usage/)里面有很多有用的提示.使用新版本的MySQL(5.7+)结合performance_schema,我们可以更轻松地解决MySQL内存分配问题。
